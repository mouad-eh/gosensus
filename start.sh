...
# Start node 1
tmux send-keys -t raft:1 "go run main.go --ip 127.0.0.1 --client-port 8001 --node-port 9001 --peers 127.0.0.1:9002,127.0.0.1:9003 --leader 127.0.0.1:9001 2>&1 | jq -C" C-m

# Start node 2
tmux send-keys -t raft:2 "go run main.go --ip 127.0.0.1 --client-port 8002 --node-port 9002 --peers 127.0.0.1:9001,127.0.0.1:9003 --leader 127.0.0.1:9001 2>&1 | jq -C" C-m

# Start node 3
tmux send-keys -t raft:3 "go run main.go --ip 127.0.0.1 --client-port 8003 --node-port 9003 --peers 127.0.0.1:9001,127.0.0.1:9002 --leader 127.0.0.1:9001 2>&1 | jq -C" C-m
